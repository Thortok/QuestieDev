...
function PT:OnPlayerEnteringWorld() -- Quick dirty hack to start proximity timer
    self:UnregisterEvent("PLAYER_ENTERING_WORLD");
	-- if Questie.db.global.trackerSortObjectives == "byProximity" then -- TODO: add the 'byProximity' setting
	  UpdateQuestProximityTimer(); 
	-- end
end

PT:RegisterEvent("PLAYER_ENTERING_WORLD", "OnPlayerEnteringWorld") -- Quick dirty hack to start proximity timer on entering the world
...
function UpdateQuestProximityTimer() -- Check location often and update if you've moved
	-- if Questie.db.global.trackerSortObjectives == "byProximity" then -- TODO: add the 'byProximity' setting
	...
	-- elseif questProximityTimer then -- Untested, TODO: Disable when 'byProximity' is off
    --    playerPosition = nil;
...
function GetDistanceToClosestObjective(questID) -- main function for proximity sorting
        ...
        elseif quest.Objectives then
            for _, objective in pairs(quest.Objectives) do
                for _, v in pairs(objective.AlreadySpawned) do
					...
					if mapRef.UiMapID then
                        local _, worldPosition = C_Map.GetWorldPosFromMapPos(mapRef.UiMapID, {
                            x = mapRef.x / 100,
                            y = mapRef.y / 100
                        });
    ...
    -- TODO: Find a way to avoid needing the quest object from the log... -- This TODO was copied from BQT
...
function QuestieTracker:Update()
    ...
    if true then -- Proximity sorting hack, currently overrides all other settings, needs a setting
--  if Questie.db.global.trackerSortObjectives == "byProximity" then	-- TODO: Add the 'byProximity' setting, remove line above, uncomment this line
        ...
        end) -- End proximity sorting
	
	elseif Questie.db.global.trackerSortObjectives == "byComplete" then
        table.sort(order, function(a, b)
